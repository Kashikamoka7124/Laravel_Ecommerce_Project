...
private function recursivecategories1(){
  ...
  //currentstock
	function productQuantity($data){
		...
		if(!empty($inventory_ref_id) && isset($inventory_ref_id[1])){
		 $minMax = DB::table('manage_min_max')->where([['inventory_ref_id', $inventory_ref_id[1]],['products_id',$products_id]])->get();
    }else{
			$minMax = '';
		}
